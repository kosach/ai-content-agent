...
async function handleCallbackQuery(ctx) {
    ...
    observability_1.logger.info({ callbackData, userId: ctx.from.id }, 'Received callback query');
    try {
        // Parse callback data
        ...
        switch (action) {
            ...
            case 'cancel':
                ...
                break;
            default:
                observability_1.logger.warn({ callbackData }, 'Unknown callback action');
                await ctx.answerCbQuery('Unknown action');
        }
    }
    catch (error) {
        observability_1.logger.error({ error, callbackData }, 'Failed to handle callback query');
 ...
 */
async function handleApprove(ctx, draftPackageId) {
    observability_1.logger.info({ draftPackageId }, 'Handling approve callback');
    // Get draft package
    const draftPackage = await database_1.database.draftPackage.findUnique({
    ...
    if (!draftPackage) {
        observability_1.logger.error({ draftPackageId }, 'Draft package not found');
    ...
    await database_1.database.contentSession.update({
        where: { id: session.id },
        data: { status: core_1.SessionStatus.APPROVED },
    });
    await ctx.answerCbQuery('✅ Approved!');
    await ctx.reply('✅ Great! Publishing to YouTube and Facebook...');
    // Check for connected accounts
    const youtubeAccount = session.brandProfile.connectedAccounts.find((acc) => acc.platform === 'YOUTUBE');
    const facebookAccount = session.brandProfile.connectedAccounts.find((acc) => acc.platform === 'FACEBOOK');
    if (!youtubeAccount && !facebookAccount) {
        await ctx.reply('⚠️ No connected accounts found.\n\n' +
    ...
    const publishJobs = [];
    if (youtubeAccount) {
        const youtubeJob = await database_1.database.publishJob.create({
            data: {
        ...
        const queueJobId = await queue_1.queueService.enqueue(core_1.JobType.PUBLISH_YOUTUBE, jobData);
        publishJobs.push({ platform: 'YouTube', jobId: queueJobId });
        observability_1.logger.info({ publishJobId: youtubeJob.id, queueJobId }, 'YouTube publish job created');
    }
    if (facebookAccount) {
        const facebookJob = await database_1.database.publishJob.create({
            data: {
                sessionId: session.id,
                draftPackageId: draftPackage.id,
                platform: 'FACEBOOK',
        ...
        const queueJobId = await queue_1.queueService.enqueue(core_1.JobType.PUBLISH_FACEBOOK, jobData);
        publishJobs.push({ platform: 'Facebook', jobId: queueJobId });
        observability_1.logger.info({ publishJobId: facebookJob.id, queueJobId }, 'Facebook publish job created');
    }
    // Update session status to PUBLISHING
    await database_1.database.contentSession.update({
        where: { id: session.id },
        data: { status: core_1.SessionStatus.PUBLISHING },
    });
    const platformsList = publishJobs.map((j) => j.platform).join(' and ');
    await ctx.reply(`🚀 Publishing to ${platformsList}...\n\n` +
        'You will be notified when your content is live!');
    observability_1.logger.info({ sessionId: session.id, publishJobs }, 'Publish jobs enqueued');
}
/**
 * Handle "Revise" button click
 */
async function handleRevise(ctx, draftPackageId) {
    observability_1.logger.info({ draftPackageId }, 'Handling revise callback');
    // Get draft package
    const draftPackage = await database_1.database.draftPackage.findUnique({
        where: { id: draftPackageId },
        include: {
    ...
    await ctx.reply('✏️ What would you like me to change?\n\n' +
    ...
    observability_1.logger.info({ draftPackageId }, 'Revision mode activated');
}
/**
 * Handle "Cancel" button click
 */
async function handleCancel(ctx, sessionId) {
    observability_1.logger.info({ sessionId }, 'Handling cancel callback');
    // Get session
    ...
    observability_1.logger.info({ sessionId }, 'Session cancelled');
}
